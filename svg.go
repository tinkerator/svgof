...
// New is the SVG constructor, specifying the io.Writer where the
// generated SVG is written and the number of digits after the decimal
// place in generated data (which defaults to 2, but can be overridden
// prior to calling (*SVG) Start*()).
func New(w io.Writer) *SVG {
	return &SVG{Writer: w, Decimals: 2}
}
...
// Start begins the SVG document with the width w and height h.  Other
// attributes may be optionally added, for example viewbox or
// additional namespaces Standard Reference:
...
// Startunit begins the SVG document, with width and height in the
// specified units Other attributes may be optionally added, for
// example viewbox or additional namespaces
...
// Startpercent begins the SVG document, with width and height as
// percentages Other attributes may be optionally added, for example
// viewbox or additional namespaces
...
// Startview begins the SVG document, with the specified width,
// height, and viewbox Other attributes may be optionally added, for
// example viewbox or additional namespaces
...
// linkembed defines an element with a specified type, (for example
// "application/javascript", or "text/css").  if the first variadic
// argument is a link, use only the link reference.  Otherwise, treat
// those arguments as the text of the script (marked up as CDATA).  if
// no data is specified, just close the element
...
// Script defines a script with a specified type, (for example
...
// TranslateRotate translates the coordinate system to (x,y), then
// rotates to r degrees, end with Gend()
func (svg *SVG) TranslateRotate(x, y float64, r float64) {
	d := svg.Decimals
	svg.Gtransform(translate(x, y, d) + " " + rotate(r))
}

// RotateTranslate rotates the coordinate system r degrees, then
// translates to (x,y), end with Gend()
...
func (svg *SVG) ClipPath(s ...string) {
	svg.printf(`<clipPath %s`, endstyle(s, `>`))
}
...
// Use places the object referenced at link at the location x, y, with
// optional style.  Standard Reference:
...
// Ellipse centered at x,y, centered at x,y with radii w, and h, with
// optional style.  Standard Reference:
...
// Polygon draws a series of line segments using an array of x, y
// coordinates, with optional style.  Standard Reference:
// http://www.w3.org/TR/SVG11/shapes.html#PolygonElement
func (svg *SVG) Polygon(x []float64, y []float64, s ...string) {
	svg.poly(x, y, "polygon", s...)
}

// Rect draws a rectangle with upper left-hand corner at x,y, with
// width w, and height h, with optional style Standard Reference:
...
// CenterRect draws a rectangle with its center at x,y, with width w,
// and height h, with optional style
func (svg *SVG) CenterRect(x float64, y float64, w float64, h float64, s ...string) {
	svg.Rect(x-(w/2), y-(h/2), w, h, s...)
}

// Roundrect draws a rounded rectangle with upper the left-hand corner
// at x,y, with width w, and height h. The radii for the rounded
// portion are specified by rx (width), and ry (height).  Style is
...
// Square draws a square with upper left corner at x,y with sides of
// length l, with optional style.
...
// Path draws an arbitrary path, the caller is responsible for
// structuring the path data
func (svg *SVG) Path(d string, s ...string) {
	svg.printf(`<path d="%s" %s`, d, endstyle(s, emptyclose))
}

// Arc draws an elliptical arc, with optional style, beginning
// coordinate at sx,sy, ending coordinate at ex, ey width and height
// of the arc are specified by ax, ay, the x axis rotation is r if
// sweep is true, then the arc will be drawn in a "positive-angle"
// direction (clockwise), if false, the arc is drawn counterclockwise.
// if large is true, the arc sweep angle is greater than or equal to
// 180 degrees, otherwise the arc sweep is less than 180 degrees
...
// Bezier draws a cubic bezier curve, with optional style, beginning
// at sx,sy, ending at ex,ey with control points at cx,cy and px,py.
// Standard Reference:
...
// Qbez draws a quadratic bezier curver, with optional style beginning
// at sx,sy, ending at ex, sy with control points at cx, cy Standard
// Reference:
...
// Qbezier draws a Quadratic Bezier curve, with optional style,
// beginning at sx, sy, ending at tx,ty with control points are at
// cx,cy, ex,ey.  Standard Reference:
// http://www.w3.org/TR/SVG11/paths.html#PathDataQuadraticBezierCommands
...
// Image places at x,y (upper left hand corner), the image with width
// w, and height h, referenced at link, with optional style.  Standard
// Reference: http://www.w3.org/TR/SVG11/struct.html#ImageElement
...
// Text places the specified text, t at x,y according to the style
// specified in s Standard Reference:
...
// Textspan begins text, assuming a tspan will be included, end with
// TextEnd() Standard Reference:
...
// TextEnd ends spanned text Standard Reference:
// https://www.w3.org/TR/SVG11/text.html#TSpanElement
func (svg *SVG) TextEnd() {
	svg.println(`</text>`)
}

// Textpath places text optionally styled text along a previously
// defined path Standard Reference:
...
// Textlines places a series of lines of text starting at x,y, at the
// specified size, fill, and alignment.  Each line is spaced according
// to the spacing argument
...
// RGB specifies a fill color in terms of a (r)ed, (g)reen, (b)lue
// triple.  Standard reference: http://www.w3.org/TR/css3-color/
func (svg *SVG) RGB(r int, g int, b int) string {
	return fmt.Sprintf(`fill:rgb(%d,%d,%d)`, r, g, b)
}

// RGBA specifies a fill color in terms of a (r)ed, (g)reen, (b)lue
// triple and opacity (range [0,1]).
...
// along the vector defined by (x1,y1), and (x2,y2).  The stop color
// sequence defined in sc. Coordinates are expressed as percentages.
...
// centered at (cx,cy), with a radius of r.  (fx, fy) define the
// location of the focal point of the light source.  The stop color
// sequence defined in sc.  Coordinates are expressed as percentages.
...
// stopcolor is a utility function used by the gradient functions to
// define a sequence of offsets (expressed as percentages) and colors
...
// Filter Effects: Most functions have common attributes (in, in2,
// result) defined in type Filterspec used as a common first argument.
...
// FeColorMatrixHue specifies a color matrix filter primitive, with
// hue rotation values. Standard reference:
...
// FeColorMatrixSaturate specifies a color matrix filter primitive,
// with saturation values.  Standard reference:
// http://www.w3.org/TR/SVG11/filters.html#feColorMatrixElement
...
// FeColorMatrixLuminence specifies a color matrix filter primitive,
// with luminence values.  Standard reference:
// http://www.w3.org/TR/SVG11/filters.html#feColorMatrixElement
...
// FeComponentTransfer begins a feComponent filter element.  Standard
// reference:
...
// FeComposite specifies a feComposite filter primitive.  Standard
// reference:
...
// FeDiffuseLighting specifies a diffuse lighting filter primitive, a
// container for light source elements, end with DiffuseEnd().
// Standard reference:
// http://www.w3.org/TR/SVG11/filters.html#feComponentTransferElement
...
// FeFunc{linear|Gamma|Table|Discrete} specify various types of
// feFunc{R|G|B|A} filter primitives Standard reference:
// http://www.w3.org/TR/SVG11/filters.html#feComponentTransferElement

// FeFuncLinear specifies a linear style function for the
// feFunc{R|G|B|A} filter element.  Standard reference:
// http://www.w3.org/TR/SVG11/filters.html#feComponentTransferElement
...
// FeFuncGamma specifies the curve values for gamma correction for the
// feFunc{R|G|B|A} filter element.  Standard reference:
// http://www.w3.org/TR/SVG11/filters.html#feComponentTransferElement
...
// FeFuncTable specifies the table of values for the feFunc{R|G|B|A}
// filter element.  Standard reference:
// http://www.w3.org/TR/SVG11/filters.html#feComponentTransferElement
...
// FeFuncDiscrete specifies the discrete values for the
// feFunc{R|G|B|A} filter element.  Standard reference:
// http://www.w3.org/TR/SVG11/filters.html#feComponentTransferElement
...
// FeMerge specifies a feMerge filter primitive, containing feMerge
// elements.  Standard reference:
...
// FeOffset specifies the feOffset filter primitive.  Standard
// reference: http://www.w3.org/TR/SVG11/filters.html#feOffsetElement
...
// FePointLight specifies a fePpointLight filter primitive.  Standard
// reference:
...
// FeSpotLight specifies a feSpotLight filter primitive.  Standard
// reference:
...
// FeTile specifies the tile utility filter primitive.  Standard
// reference: http://www.w3.org/TR/SVG11/filters.html#feTileElement
func (svg *SVG) FeTile(fs Filterspec, in string, s ...string) {
	svg.printf(`<feTile %s %s`, fsattr(fs), endstyle(s, emptyclose))
}

// FeTurbulence specifies a turbulence filter primitive.  Standard
// reference:
...
// The animation starts at coordinate from, terminates at to, and
...
// AnimateMotion animates the referenced object along the specified
// path.
...
// Grid draws a grid at the specified coordinate, dimensions, and
// spacing, with optional style.
...
// sce makes start, center, end coordinates string for animate
// transformations.
...
// endstyle modifies an SVG object, with either a series of
// name="value" pairs, or a single string containing a style.
...
func islink(link string) bool {
	return strings.HasPrefix(link, "https://") || strings.HasPrefix(link, "http://") || strings.HasPrefix(link, "#") || strings.HasPrefix(link, "../") || strings.HasPrefix(link, "./")
}

// group returns a group element.
func group(tag string, value string) string {
	return fmt.Sprintf(`<g %s="%s">`, tag, value)
}

// scale return the scale string for the transform.
func scale(n float64) string {
	return fmt.Sprintf(`scale(%g)`, n)
}

// scaleXY return the scale string for the transform.
func scaleXY(dx, dy float64) string {
	return fmt.Sprintf(`scale(%g,%g)`, dx, dy)
}

// skewx returns the skewX string for the transform.
func skewX(angle float64) string {
	return fmt.Sprintf(`skewX(%g)`, angle)
}

// skewx returns the skewX string for the transform.
func skewY(angle float64) string {
	return fmt.Sprintf(`skewY(%g)`, angle)
}

// rotate returns the rotate string for the transform.
func rotate(r float64) string {
	return fmt.Sprintf(`rotate(%g)`, r)
}

// translate returns the translate string for the transform.
func translate(x, y float64, d int) string {
	return fmt.Sprintf(`translate(%.*f,%.*f)`, d, x, d, y)
}

// coord returns a coordinate string.
func coord(x float64, y float64, d int) string {
	return fmt.Sprintf(`%.*f,%.*f`, d, x, d, y)
}

// ptag returns the beginning of the path element.
func ptag(x float64, y float64, d int) string {
	return fmt.Sprintf(`<path d="M%s`, coord(x, y, d))
}

// loc returns the x and y coordinate attributes.
func loc(x float64, y float64, d int) string {
	return fmt.Sprintf(`x="%.*f" y="%.*f"`, d, x, d, y)
}

// href returns the href name and attribute.
func href(s string) string {
	return fmt.Sprintf(`xlink:href="%s"`, s)
}

// dim returns the dimension string (x, y coordinates and width,
// height).
func dim(x float64, y float64, w float64, h float64, d int) string {
	return fmt.Sprintf(`x="%.*f" y="%.*f" width="%.*f" height="%.*f"`, d, x, d, y, d, w, d, h)
}

// fsattr returns the XML attribute representation of a filterspec,
